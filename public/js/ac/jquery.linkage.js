...
function LinkageSelect(options) {
	 ...
	 * @param {Object} value
	 */
	function _bind(element , title , value) {
		...
		if (self_id > 0) {
			bindEls[self_id-1].element.change(function() {
				_fill(title , element , bindEls[self_id].key);
			});
		}
				...
				if (typeof bindEls[self_id+1] == 'undefined') {
					_find(self_key , function(key , json) {
						if (json) {
							...
							element.after(el);
							_bind(el , '');
							_fill(title , bindEls[self_id+1].element , key , json);
						}
					});
				}
			}
		})
		_fill(title , element , key , value);
	 ...
	 * @param {Object} value
	 */
	function _fill(title , element , key , value) {
	...
	return {
		...
		// 绑定元素
		bind	: function(element , title , value) {
			if (typeof element != 'object')
				element	= _getEl(element);
			value	= value?value:'';
			
			_bind(element , title , value);
